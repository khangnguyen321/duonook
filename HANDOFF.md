...
- Shared-glance publication: implementation commit `aeeae4f` was pushed to `origin/main` on August 21, 2026. The live health endpoint remained healthy, but `/` still referenced the previous dashboard assets (`index-H2x38Hyx.js` and `index-DJ_xDHr0.css`) rather than this checkpoint's build assets (`index-CWR-QhoE.js` and `index-Cdwh7sIH.css`) after a bounded follow-up check. NoiGate's automatic rebuild is still not updating the live service; manually redeploy or repair its push trigger before treating weather and location as live.
